...
export function GroupSelector({ onChange }) {
  ...
  const { data, isLoading } = useApiQuery("/api/dashboard/groups");

  // When group data changes, notify parent
  useEffect(() => {
    if (!selectedGroupId || !data || !onChange) return;
    const selected = data.find((g) => g.id === selectedGroupId);
    if (selected) {
      onChange(selected);
    }
  }, [data, selectedGroupId, onChange]);
